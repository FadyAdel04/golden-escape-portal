  ...
  return <div className="min-h-screen bg-white">
      ...
      <div className="bg-navy text-white py-8">
        <div className="container mx-auto px-4">
          <Link to="/">
            <Button variant="outline" className="mb-4 border-white hover:bg-white text-zinc-950">
              <ArrowLeft className="h-4 w-4 mr-2" />
              Back to Rooms
            </Button>
          </Link>
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="grid lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-4">
            {room.room_images && room.room_images.length > 0 ? <Carousel className="w-full">
                <CarouselContent>
                  {room.room_images.sort((a, b) => a.display_order - b.display_order).map(image => <CarouselItem key={image.id}>
                        <div className="aspect-[4/3] overflow-hidden rounded-lg">
                          <img src={image.image_url} alt={image.alt_text || room.title} className="w-full h-full object-cover" />
                        </div>
                      </CarouselItem>)}
                </CarouselContent>
                <CarouselPrevious />
                <CarouselNext />
              </Carousel> : <div className="aspect-[4/3] bg-gray-200 rounded-lg flex items-center justify-center">
          ...
          <div className="space-y-6">
            <div>
              ...
              {room.description && <p className="text-gray-700 text-lg leading-relaxed">{room.description}</p>}
            </div>

            {/* Features */}
            {room.features && room.features.length > 0 && <div>
                <h3 className="text-xl font-bold text-navy mb-3">Room Features</h3>
                <div className="grid grid-cols-2 gap-3">
                  {room.features.map((feature, index) => {
                const IconComponent = getFeatureIcon(feature);
                return <div key={index} className="flex items-center text-gray-600 bg-beige/50 px-3 py-2 rounded-lg">
            ...
            <div className="border-t pt-6">
              <Button className="w-full bg-gold hover:bg-gold/90 text-white py-3 text-lg font-semibold" disabled={!room.availability}>
