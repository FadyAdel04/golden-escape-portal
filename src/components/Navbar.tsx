...
const Navbar = () => {
  ...
  return <nav className={`fixed w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-white shadow-md py-3' : 'bg-transparent py-5'}`}>
      <div className="container mx-auto px-4 md:px-6">
        ...
        {/* Mobile Menu */}
        {mobileMenuOpen && <div className="md:hidden mt-4 bg-white rounded-lg shadow-lg py-4 px-2 absolute left-4 right-4 animate-fade-in">
            <div className="flex flex-col space-y-3">
              {navLinks.map(link => <a key={link.name} href={link.href} className="text-navy hover:text-gold px-4 py-2 rounded-md transition-colors" onClick={() => setMobileMenuOpen(false)}>
                  {link.name}
                </a>)}
